...
large language models for help---yet it is unclear whether LLMs can
...
wrong, but that \emph{any} use of the age signal---in either
direction---consistently fails to beat the baseline that ignores it
entirely.
...
performance.
Larger models occasionally discover high-quality solutions---diamonds
in the rough---that smaller models never reach, even though their
typical output is no better.
...
load balancing.
If so, the largest models carry latent knowledge of the correct
strategy---visible in the rare trials that get it right---but cannot
sample it consistently because the policy was not shaped by feedback
from this problem class.
...
stated mean service time---evidence that the model is reading and
applying the failure description, not just pattern-matching to the
interface.
...
A clean ablation---add an explicit thundering-herd description to
Experiment~1, or withhold the ERW-freeze explanation from
