...
large language models for advice. Whether LLMs can reason about the
stochastic properties that make these problems hard is, however, unclear.
We study this through two controlled experiments in which six LLMs
generate dispatch functions zero-shot for a server farm under
heartbeat staleness and under noisy SRPT routing.
The results are consistent across both experiments.
No model reliably beats the best classical baseline in median
performance, and naive deployment of LLM-generated functions
frequently performs \emph{worse} than the algorithm being replaced.
Scale determines code validity below roughly 8B parameters but does
not predict median quality above that threshold.
Larger models occasionally discover high-quality solutions that
smaller models never reach, even when their typical output is no
better.
Providing an explicit description of the failure mode increases the
rate of correct solutions but does not produce reliable improvements.
These results suggest that LLMs carry useful algorithmic knowledge
but cannot yet be trusted to improve a deployed load-balancing
